...
project=ezdap-adapters
description="Ready-made DAP adapter definitions for ezdap.nvim"
...
awk -v textwidth=78 '
    # A derived tag is heading text, so it can hold regex metacharacters -- the
    # "c++" in "C / C++ / Rust" would otherwise be read as a repeat.
    function esc(s) {
        gsub(/[][\\^$.|?*+(){}\/]/, "\\\\&", s)
        return s
    }
    ...
    }
    {
        line = $0
        for (k in map) {
            gsub("\\*" esc(k) "\\*", "*" map[k] "*", line)
            gsub("\\|" esc(k) "\\|", "|" map[k] "|", line)
        }
        # Re-align the trailing tag: back to where it was, or to the text width
        # when the derived tag had pushed the line past it.
...
' "$work/tagmap" "$work/doc/$project.txt" |

# pandoc closes a table with its last row, so whatever follows it -- the next
# mode, a paragraph -- is left hard against the table. Put the blank line back.
awk '
    /^ +-{3,}/ { intable = 1 }
    /^[ \t]*$/ { intable = 0 }
    intable && /^[^ \t]/ { print ""; intable = 0 }
    { print }
' > "$work/$project.txt"
