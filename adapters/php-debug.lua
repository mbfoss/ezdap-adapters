...
local _common_inputs = {
    ...
    path_mappings      = { type = "table", format = "map", description = "source path mappings, server=local" },
    stop_on_entry      = { type = "boolean", description = "break at the first line" },
    ignore             = { type = "table", format = "list", description = "globs whose errors are ignored (default **/vendor/**/*.php)" },
    ignore_exceptions  = { type = "table", format = "list", description = "exception class names to ignore" },
    skip_files         = { type = "table", format = "list", description = "globs to skip when stepping (default **/vendor/**)" },
    skip_entry_paths   = { type = "table", format = "list", description = "globs that abandon the session when the entry file matches" },
    max_connections    = { type = "integer", description = "maximum parallel debug sessions (0 = unlimited)" },
    xdebug_settings    = { type = "table", format = "map", description = "DBGP feature overrides, e.g. max_depth=3,max_children=100" },
...
local _profiles = {
    ...
    launch_program = {
        ...
        inputs = _inputs {
            ...
            runtime_args       = { type = "table", format = "list", description = "arguments passed to php, e.g. -dxdebug.mode=debug,-dxdebug.start_with_request=yes" },
